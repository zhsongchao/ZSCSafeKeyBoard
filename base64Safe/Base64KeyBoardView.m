        ...
        switch (self.TextSelectPostion) {
            ...
            case TextFiledSecond:
            {
                if (self.delegate && [self.delegate respondsToSelector:@selector(getSecondKeyBoardViewValueFromButton:DidSelectButTag:)]) {
                    [self.delegate  getSecondKeyBoardViewValueFromButton:self.JumpStrSecond DidSelectButTag:sender.tag];
                    __weak typeof(self) weakSelf = self;
                    if (self.BlockChnagkeyBoard) {
                        weakSelf.BlockChnagkeyBoard([weakSelf CharacterKeyBoardShowView]);
                    }
...
-(void)CharacterDidSelect:(UIButton *)sender {
    if (sender.tag == 19) {
        ...
        switch (self.TextSelectPostion) {
            ...
            {
                if (self.delegate && [self.delegate respondsToSelector:@selector(getSecondKeyBoardViewValueFromButton:DidSelectButTag:)]) {
                    [self.delegate  getSecondKeyBoardViewValueFromButton:self.JumpStrSecond DidSelectButTag:[sender.OrderTags integerValue]];
                    __weak typeof(self) weakSelf = self;
                    if (self.BlockChnagkeyBoard) {
                        weakSelf.BlockChnagkeyBoard([weakSelf NumberKeyBoardShowView]);
                    }
...
//                [self runDelegateFromButtonTag:self.JumpStr andButtonTage:[sender.OrderTags integerValue]];
            ...
            case TextFiledSecond:
            {
//                self.JumpStrSecond = [self.ConnectStrSecond stringByAppendingString:@""];
//                self.ConnectStrSecond = self.JumpStrSecond;
//                [self runSecondDelegateFromButtonTag:self.JumpStrSecond andButtonTage:[sender.OrderTags integerValue]];
            }
                break;
            case TextFiledOther:
            {
        ...
        switch (self.TextSelectPostion) {
            ...
            case TextFiledOther:
            {
                if (self.OtherselectState) {
                    NSString * baseStr = __BASE64([self.SmallCharacterArry objectAtIndex:sender.tag]);
                    self.OtherJumpStr = [[self.OtherConnectStr stringByAppendingString:baseStr] stringByAppendingString:@","];
                    self.OtherConnectStr = self.OtherJumpStr;
                    self.OtherJumpStr = [self.OtherJumpStr substringWithRange:NSMakeRange(0, self.OtherJumpStr.length - 1)];
                    [self runOtherDelegateFromButtonTag:self.OtherJumpStr andButtonTage:[sender.OrderTags integerValue]];
                    ...
                    self.OtherConnectStr = self.OtherJumpStr;
                    self.OtherJumpStr = [self.OtherJumpStr substringWithRange:NSMakeRange(0, self.OtherJumpStr.length - 1)];
                    [self runDelegateFromButtonTag:self.OtherJumpStr andButtonTage:[sender.OrderTags integerValue]];
...
-(void)runSecondDelegateFromButtonTag:(NSString *)ButtonText andButtonTage:(NSInteger)TageValue {
    if (self.delegate && [self.delegate respondsToSelector:@selector(getSecondKeyBoardViewValueFromButton:DidSelectButTag:)]) {
        NSLog(@"第二个文本输入框加密后获取文本框内容%@",ButtonText);
    ...
    switch (self.TextSelectPostion) {
        ...
        case TextFiledSecond:
        {
            if (![self.ConnectStrSecond isEqualToString:@""]) {
                ...
                /**赋值删除完成标志*/
                self.DeleteDoneSecond = IsStrEmpty(self.JumpStrSecond);
                [self runSecondDelegateFromButtonTag:resShowStr andButtonTage:11];
            }
        }
            break;
        default:
